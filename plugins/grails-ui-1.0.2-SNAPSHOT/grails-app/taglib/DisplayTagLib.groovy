class DisplayTagLib {
    ...
    def dataTable = {attrs ->
        ...
        if (!attrs.columnDefs.find {it.key == sortedBy}) {
            throw new GrailsUIException("The GrailsUI dataTable cannot be defined with a 'sortedBy' value of '${sortedBy}' because is "
                + "does not exist in the column definition.  To fix, either update the columnDefs attribute to contain the sortedBy "
                + "value or change sortedBy to refer to a column defined in 'columnDefs'.")
        }
        ...
        out << """
            ...
            YAHOO.util.Event.onDOMReady(function () {
                ...
                GRAILSUI.${jsid} = new GRAILSUI.DataTable('dt_div_${id}', myColumnDefs, ${jsid}_ds, '${paramQueryString}', {
                    ...
                    dynamicData            : true,
                    sortedBy               : {key: "${sortedBy}", dir: YAHOO.widget.DataTable.CLASS_ASC},
